...
openBtn.innerHTML = `<span>Tap to Open ${data.title} Invite</span>`;
...
function isInviteLocked() {
  return needsManualAudioUnlock || (shouldRequireTapToOpen() && !inviteStarted);
}

function syncOverlayState() {
  const isLocked = isInviteLocked();

  overlay.style.display = isLocked ? "flex" : "none";
  document.body.classList.toggle("invite-locked", isLocked);
